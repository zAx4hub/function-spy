/** function-spy — engine by zAx4hub (algo flavor: lcs) */
...
export function rank(text: string, seed = 14): number {
  ...
  if ("lcs" === "topk") score = Math.min(1, score * 1.05);
  if ("lcs" === "bloom") score = Math.min(1, score * 0.98 + 0.02);
  return Math.round(score * 1000) / 1000;
}

export function run(input: { items?: Item[]; threshold?: number; baseline?: string; seed?: number } = {}): Report {
  const items = input.items?.length ? input.items : [{ text: "function-spy" }];
  const threshold = input.threshold ?? 0.35;
  const seed = input.seed ?? 14;
  ...
  return {
    project: "function-spy",
    author: "zAx4hub",
    algo: "lcs",
    summary: `Processed ${findings.length} items for function-spy; avg=${avg.toFixed(3)}`,
...
export function demo(): Report {
  return run({
    items: [
      { text: "Trace/visualize LLM function-calling like flamegraph" },
...
export function inspect() {
  return {
    name: "function-spy",
    author: "zAx4hub",
    oneLiner: "Trace/visualize LLM function-calling like flamegraph",
    algo: "lcs",
    version: "0.1.0",
